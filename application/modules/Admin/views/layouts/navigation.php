    ...
    <?php echo form_open(base_url() . $route.'/search-' . $route, array("class" => "form-inline my-2 my-lg-0"));?>
     ...
     foreach ($search_options as $search_option_array) {?>
			<select class="form-control custom-select2 p-3" name="<?php echo $search_option_array[0]; ?>">
				<option value="">Select
					<?php echo $search_option_array[2]; ?>
				</option>
				<?php foreach ($search_option_array[1] as $key => $search_option) {
        ?>
				<option value="<?php echo $search_option['id']; ?>">
					<?php echo $search_option['name']; ?>
				</option>
				<?php }?>
			</select>
			<?php }
      ?>
			...
			<a href="<?php echo base_url(); ?><?php echo $route; ?>/close-search" class="btn btn-outline-danger my-2 my-sm-0 ml-sm-2"><i
...
>>>>>>> 5c45dceb7e487acec22a9cf05921ac041e2f92b9
