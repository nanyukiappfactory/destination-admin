<link href="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
<script src="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
<script src="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/js/feather.min.js"></script>
<script src="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/js/Chart.min.js"></script>
<script src="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/js/dashboard.js"></script>
<link href="<?php echo base_url();?>assets/custom/themes/vendor/jquery/jquery-3.3.1.min.js" rel="stylesheet">
<script src="<?php echo base_url();?>assets/custom/themes/vendor/js/main.js"></script>
<script src="<?php echo base_url()?>assets/custom/themes/vendor/js/script.js"></script>
		
<script>
    $(document).ready(function() {
        $('#welcome_message').fadeOut(3000);
    });
</script>

<script src="<?php echo base_url();?>assets/custom/themes/vendor/bootstrap/js/jquery.datetimepicker.full.js"></script>
    <script type="text/javascript">
      $(document).ready(function(){
        $('#datetimepicker').datetimepicker();
      });
       
    </script>
    <script type="text/javascript">
      $(document).ready(function(){
        $('#datetimepicker2').datetimepicker();
      });
       
    </script>
    <!-- <script src="../bootstrap/js/bootstrap.min.js"></script> -->